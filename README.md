...
| 2 | **Structure & enrich** — VLOOKUP rep→manager/quota and brand→brand manager from two reference tables; the six failure modes; XLOOKUP note | 45 min |
| 3 | **Explore & summarize** — AutoFilter, Status Bar, SUBTOTAL; SUMIFS/COUNTIFS/AVERAGEIFS; net sales, return rate, AOV, sales by manager | 60 min |
| 4 | **Pivot & rank** — PivotTables, grouping, slicers, rep & brand-manager leaderboards | 40 min |
...
real sales KPIs (net sales, gross, return rate, AOV, quota attainment).
...
- **All course data is fully synthetic** — generated by `generate_dataset.py` from a
  fixed seed. The setting is a UAE FMCG distributor: customers are real Dubai supermarket
  *chain names* used only as realistic labels, but all codes, branches, reps, managers,
  **brands (fully fictional)**, quantities and AED values are invented. Nothing real or
  confidential ships in this repo, and the brands are deliberately unrelated to any
  Transmed portfolio.
